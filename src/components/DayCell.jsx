...
function buildEventTooltip(event) {
  ...
  if (event.isRange) {
    const startLabel = formatEventDate(normaliseDate(event.start));
    const endLabel = formatEventDate(normaliseDate(event.end));
    return `${event.label} – du ${startLabel} au ${endLabel}`.trim();
  }
  const dateLabel = formatEventDate(normaliseDate(event.start));
  return `${event.label} – ${dateLabel}`.trim();
}
